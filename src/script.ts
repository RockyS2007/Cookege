...
// for operations that require an authenticated/development user
// Login at bottom of file
export const supabase = createClient<Database>(supabaseUrl, supabaseAnonKey);

export async function setRecipeLink(recipe_link: string) {
  const response = await fetch(`http://127.0.0.1:8000/recipe_link/${recipe_link}`, 
                                {
                                  method: "PUT"
                                });

  if (!response.ok) {
    throw new Error("Failed to update recipe link")
  }
}
...
async function main() {
  ...
  // npx tsx --env-file=.env.local src/script.ts
  await setRecipeLink("TESTLINK");
}
